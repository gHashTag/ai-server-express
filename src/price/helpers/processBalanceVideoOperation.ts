...
export const processBalanceVideoOperation = async ({ videoModel, telegram_id, is_ru }: BalanceOperationProps): Promise<BalanceOperationResult> => {
  try {
    ...
    if (!videoModel || !availableModels.includes(videoModel as VideoModel)) {
      await bot.telegram.sendMessage(telegram_id, is_ru ? 'Пожалуйста, выберите корректную модель' : 'Please choose a valid model');
      return { newBalance: currentBalance, paymentAmount: 0, success: false, error: 'Invalid model' };
    }
    ...
    if (currentBalance < paymentAmount) {
      ...
      return { newBalance: currentBalance, paymentAmount, success: false, error: message };
    }
