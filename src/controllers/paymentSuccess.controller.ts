  ...
  public paymentSuccess = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const calculatedSignature = md5(
        `${this.merchantLogin}:${OutSum}:${InvId}:${encodeURIComponent(this.resultUrl2)}:${this.password2}`,
      ).toUpperCase();
      ...
      if (calculatedSignature !== SignatureValue) {
        ...
        await this.paymentService.processPayment(OutSum as string, InvId as string);
      }
